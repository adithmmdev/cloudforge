...
1. Create a new file in `backend/app/build/detectors/`.
...
3. Register your adapter in the priority list inside `backend/app/build/generator.py` or the adapter registry.
4. Add Jinja2 Dockerfile templates in `backend/app/templates/`. These templates should use the entry-point variables returned by your `detect` function.
